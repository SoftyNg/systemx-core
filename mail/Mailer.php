 ...
 */
 ...
 namespace systemx\SystemxCore\mail;

 use systemx\SystemxCore\exception\MailException;
/**
    ...
    public function setLanguage($langcode = 'en', $lang_path = '')
    {
        ...
        if ('en' !== $langcode) {
            ...
            foreach ($langcodes as $code) {
                $lang_file = $lang_path . 'mailer.lang-' . $code . '.php';
            ...
            if ($foundFile === false) {
                ...
                foreach ($lines as $line) {
                    ...
                    //$Mailer_LANG['authenticate'] = 'SMTP-Fehler: Authentifizierung fehlgeschlagen.';
                    //These files are parsed as text and not PHP so as to avoid the possibility of code injection
                    //See https://blog.stevenlevithan.com/archives/match-quoted-string
                    $matches = [];
                    if (
                        preg_match(
                            '/^\$Mailer_LANG\[\'([a-z\d_]+)\'\]\s*=\s*(["\'])(.+)*?\2;/',
                            $line,
                            $matches
                        ) &&
                        //Ignore unknown translation keys
                        array_key_exists($matches[1], $Mailer_LANG)
                    ) {
                        //Overwrite language-specific strings so we'll never have missing translation keys.
                        $Mailer_LANG[$matches[1]] = (string)$matches[3];
                    }
                }
            }
        }
